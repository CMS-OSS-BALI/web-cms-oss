...
function CollegeFormModal({
  ...
  return (
    <Modal
      title={mode === "edit" ? "Edit College" : "Add College"}
      open={open}
      centered
      width={900}
      onCancel={onCancel}
...
/* ===== View Modal ===== */
function CollegeViewModal({ open, data, onClose }) {
  ...
  return (
    <Modal
      ...
      centered
      width={960}
      title={data?.name || "Detail College"}
      footer={
...
/* ===== Card ===== */
function CollegeCard({ p, onView, onEdit, onDelete }) {
...
/* ===== Main ===== */
export default function CollegesContent(props) {
  const {
    loading,
    colleges = [],
    q,
    ...
    total = 0,
    error,
    // message (not shown to avoid duplicate toasts)
    fetchColleges,
    createCollege,
    updateCollege,
    deleteCollege: deleteCollegeVm,
  } = props;
  ...
  const handleSubmit = async (payload) => {
    ...
    const res =
      mode === "edit"
        ? await updateCollege(editing.id, payload)
    ...
    if (res?.ok) {
      api.success({
        key: "college-save",
        message: mode === "edit" ? "College diperbarui" : "College ditambahkan",
  ...
  const onSearch = () => fetchColleges({ page: 1, q, country, type, perPage });

  const onReset = () => {
    setQ("");
    setCountry("");
    setType("");
    fetchColleges({ page: 1, q: "", country: "", type: "", perPage });
  };

  const onPageChange = (p) => {
    setPage(p);
    fetchColleges({ page: p, perPage, q, country, type });
  };
      ...
      <div style={pageWrapStyle}>
        ...
        >
          ...
          >
            <div>
              ...
              <Text type="secondary">
                Kelola data kampus. Total {total} records.
              </Text>
            </div>
            <Button
              type="primary"
              ...
              <Col xs={24} sm={8} md={6} lg={4} xl={3}>
                <Select
                  ...
                  onChange={(v) => {
                    setPerPage(v);
                    setPage(1);
                    fetchColleges({ page: 1, perPage: v, q, country, type });
                  }}
        ...
        <Card styles={{ body: { padding: 16 } }} style={{ ...darkCardStyle }}>
          {loading ? (
            ...
            <Empty description="Belum ada data kampus" />
          ) : (
            <Row gutter={[16, 16]}>
              {colleges.map((p) => (
                <Col key={p.id} xs={24} sm={12} md={12} lg={8} xl={6}>
                  <CollegeCard
                    ...
                    onEdit={() => openEdit(p)}
                    onDelete={async () => {
                      const { ok, error: err } = await deleteCollegeVm(p.id);
                      if (ok) {
                        notification.success({
                          message: "College dihapus",
