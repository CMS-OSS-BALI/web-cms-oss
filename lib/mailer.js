...
const HEADER_LOGO_CID = "ticket-header-logo";
const DEFAULT_HEADER_LOGO = "/logo-oss-cube-basic.svg";
...
async function loadInlineAsset(relPath, cid, contentType) {
  if (!relPath) return null;
  try {
    const abs = path.join(process.cwd(), "public", String(relPath).replace(/^\//, ""));
    const buf = await readFile(abs);
    return {
      filename: path.basename(abs),
      content: buf,
      cid,
      contentType: contentType || guessMime(abs),
...
/* =========================================================
   Notifikasi Lead Baru (ke admin)
========================================================= */
export async function sendNewLeadNotificationEmail({ to, lead, detailUrl }) {
  if (!to || !lead) return { ok: false, skipped: true };

  const subject = `[Lead Baru] ${lead.full_name || "-"}`;
  const createdAt = lead.created_at || new Date();
      ...
      <tr><td align="center">
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="background:#fff;border-radius:16px;border:1px solid #e5e7eb;overflow:hidden">
          <tr><td style="padding:20px 24px">
            <h1 style="margin:0 0 6px;font-size:20px;line-height:1.2;color:#0b56c9">Lead Baru Masuk</h1>
            <div style="color:#64748b;font-size:13px">${fmt(createdAt)}</div>
          </td></tr>

          <tr><td style="padding:0 24px 16px">
            <p style="margin:0 0 8px">Halo Admin,</p>
            <p style="margin:0 0 12px">Ada lead baru yang baru saja masuk dari website.</p>
            <table width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;font-size:14px">
              <tr>
                <td style="padding:4px 0;width:160px;color:#64748b">Nama Lengkap</td>
              ...
              <tr>
                <td style="padding:4px 0;color:#64748b">WhatsApp</td>
                <td style="padding:4px 0">${lead.whatsapp || "-"}</td>
              </tr>
              <tr>
                <td style="padding:4px 0;color:#64748b">Domisili</td>
                <td style="padding:4px 0">${lead.domicile || "-"}</td>
              </tr>
              <tr>
                <td style="padding:4px 0;color:#64748b">Pendidikan Terakhir</td>
                <td style="padding:4px 0">${lead.education_last || "-"}</td>
              </tr>
            </table>
            ...
            ${
              detailUrl
                ? `<p style="margin:12px 0 0">
                    <a href="${toAbs(
                      detailUrl
                    )}" style="display:inline-block;padding:10px 14px;border-radius:10px;background:#0b56c9;color:#fff;text-decoration:none;font-size:14px">
          ...
          <tr><td style="padding:16px 24px;background:#f8fafc;color:#64748b;font-size:12px;text-align:center">
            Email ini otomatis dikirim oleh sistem leads.
          </td></tr>
        </table>
      </td></tr>
    </table>
  ...
  const textLines = [
    ...
    detailUrl ? `Detail: ${toAbs(detailUrl)}` : null,
  ].filter(Boolean);

  const headers = {
    "X-Template-Version": "LEAD_NOTIFY_V1",
...
export async function sendTicketEmail({
  ...
  // Header logo: prefer inline CID for relative/public assets, fallback to absolute URL
  const logoCandidate = logo_url || DEFAULT_HEADER_LOGO;
  let headerLogoSrc = "";
  if (isAbsoluteUrl(logoCandidate)) {
    headerLogoSrc = toAbs(logoCandidate);
  } else {
    const inlineLogo = await loadInlineAsset(logoCandidate, HEADER_LOGO_CID);
    if (inlineLogo) {
      attachments.push(inlineLogo);
      headerLogoSrc = `cid:${HEADER_LOGO_CID}`;
    } else {
      headerLogoSrc = toAbs(logoCandidate);
    }
  }
      ...
      <tr><td align="center">
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="background:#fff;border-radius:16px;border:1px solid #e5e7eb;overflow:hidden">
          <tr><td style="padding:24px 24px 12px;text-align:center">
            ${
              headerLogoSrc
                ? `<img src="${headerLogoSrc}" alt="Logo" title="Logo" style="max-height:80px;width:auto;display:inline-block;margin-bottom:12px" />`
                : ""
            }
