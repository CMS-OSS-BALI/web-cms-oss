...
/* =========================================================
   SMTP Transporter (single instance)
========================================================= */
...
function buildTransporter() {
  ...
  return nodemailer.createTransport({
    ...
    port,
    secure: port === 465, // 465 = TLS (secure)
...
export async function sendMail({
  ...
  const from = brandName ? `${brandName} <${fromEmail}>` : fromEmail;
      ...
      <tr><td align="center">
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="background:#fff;border-radius:16px;border:1px solid #e5e7eb;overflow:hidden">
          <tr><td style="padding:24px 24px 12px;text-align:center">
            ${
              headerLogo
                ? `<img src="${headerLogo}" alt="Logo" style="max-height:80px;width:auto;display:inline-block;margin-bottom:12px" />`
                : ""
            }
